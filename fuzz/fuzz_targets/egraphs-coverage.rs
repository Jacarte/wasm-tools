...
fuzz_target!(|bytes: &[u8]| {
    ...
    #[cfg(feature = "validate-fuzz")]
    if(cfg!(feature="mutate-in-middle")) {
        // TODO disable libfuzzer feedback for wasm_mutate
        ...
        for (i, mutated_wasm) in iterator.take(100).enumerate() {
            ...
            eval_validate::validate(&mutated_wasm, &config);
        }
        
    } else {
        eval_validate::validate(&wasm, &config);
    }
...
mod eval_validate {
    ...
    pub fn validate(wasm: &[u8], config: &SwarmConfig) {
